...
class CountWorkerThread(WorkerThread):
	def run(self):
		...
		query = "https://scihub.copernicus.eu/dhus/search?q=filename:S2?_* AND beginposition:[" \
			+ str(d[1].year) + "-" + str(d[1].month) + "-" + str(d[1].day) + "T00:00:00.000Z TO " \
			+ str(d[2].year) + "-" + str(d[2].month) + "-" + str(d[2].day) + "T00:00:00.000Z] AND " \
			+ "cloudcoverpercentage:[0 TO " + str(d[3]) + "] AND footprint:\"Intersects(POLYGON(("
		...
		for i in range(((count - 1) // querySize) + 1):
			...
			self.dispatcher.addData(query + "&start=" + str(i * querySize) + "&rows=" + str(querySize))
	...
	def run(self):
		...
		for i in range(len(j['feed']['entry'])):
			...
			for s in j['feed']['entry'][i]['str']:
				if s['name'] == 'tileid':
					...
					# Level-2Ap stands for 2a pilot-project (https://sentinels.copernicus.eu/web/sentinel/news/-/article/upcoming-sentinel-2-level-2a-product-evolution)
						...
						print("error with content: " + s['content'] + " in product " + product[uid]['productname'])
						return
			...
			if product[uid]['processinglevel'] == 1:
				# Level 1 data
				try:
					product[uid]['cloudcoverage'] = float(j['feed']['entry'][i]['double']['content'])
				except TypeError as e:
					print("following product: " + product[uid]['productname'] + ", error: " + str(e))
					return
			elif product[uid]['processinglevel'] == 2:
				# Level 2 data
				product[uid]['cloudcoverage'] = float(j['feed']['entry'][i]['double'][0]['content'])
		...
		if savefile.is_file():
			with open(savefilename, 'r') as fr:
				...
				if uid in savejson:
					...
					print(uid + " already exists in " + savefilename + " (download canceled)")
		...
		if self.data[uid]['cloudcoverage'] < maxcloudcoverage:
			...
			if('b04Path' not in locals()):
				...
				try:
					...
					tree = ET.fromstring(r.text)
					if level == 2:
						missing1 = 'L2A' + tree[2][10][0][0].attrib["href"][2:].split('/')[1][3:]
						...
						b04Path = [['GRANULE', missing1, 'IMG_DATA', 'R20m', missing2 + '_B04_20m.jp2'], False]
						b08Path = [['GRANULE', missing1, 'IMG_DATA', 'R20m', missing2 + '_B8A_20m.jp2'], False]
						sclPath = [['GRANULE', missing1, 'IMG_DATA', 'R20m', missing2 + '_SCL_20m.jp2'], False]
						found = 3
					else:
						...
						b04Path = [['GRANULE', missing1, 'IMG_DATA', missing2 + '_B04.jp2'], False]
						b08Path = [['GRANULE', missing1, 'IMG_DATA', missing2 + '_B8A.jp2'], False]
			...
			try:
				...
				print("Error: " + str(e) + ", productname: " + str(product) + ", totalerrors: " + str(self.dispatcher.errorcount))
...
class ComputeWorkerThread(WorkerThread):
	def run(self):
		...
		if clouds < maxcloudcoverage:
			...
			print("calculated (" + str(self.dispatcher.calculated) + "): " + dic[uid]['productname'] + " (clouds: " + str(clouds) + ")")
			self.dispatcher.calcuLock.release()
		else:
			self.dispatcher.calcuLock.acquire()
			self.dispatcher.notcalculated += 1
			print("not calculated (" + str(self.dispatcher.notcalculated) + "): " + dic[uid]['productname'] + " (clouds: " + str(clouds) + ")")
			self.dispatcher.calcuLock.release()
		...
		# you might have more picutes of area 1 than of area 2 which could lead to noise in the data. Possible Solutions:
